  ...
  constructor(data) {
    ...
    try {
      if (data?.setupComplete || data?.setup_complete) {
        ...
        this.data = {
          text: ot.text || "",
          finished: ot.finished || false,
        };
      } else if (data?.toolCall || data?.tool_call) {
...
class GeminiLiveAPI {
  ...
  onReceiveMessage(messageEvent) {
    if (messageEvent.data instanceof Blob || messageEvent.data instanceof ArrayBuffer) {
      console.log("📦 Received BINARY message from server", messageEvent.data);
      return; 
    }
    ...
    try {
      ...
      console.log("Raw message:", messageEvent.data);
    }
  }
